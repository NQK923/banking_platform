...
public class AccountServiceClient {
    ...
    public ResponseEntity<String> exchange(HttpMethod method, String path, String body, HttpHeaders inboundHeaders) {
        HttpHeaders outboundHeaders = new HttpHeaders();
        copyForwardHeaders(inboundHeaders, outboundHeaders);
        if (body != null && !body.isBlank()) {
            outboundHeaders.setContentType(org.springframework.http.MediaType.APPLICATION_JSON);
        }
        return restTemplate.exchange(baseUrl + path, method, new HttpEntity<>(body, outboundHeaders), String.class);
    }

    private void copyForwardHeaders(HttpHeaders inbound, HttpHeaders outbound) {
